...
# # Calculate the slope of the decision boundary (perpendicular bisector)
# slope = -(mean_class4[0] - mean_class3[0]) / (mean_class4[1] - mean_class3[1])

# # Calculate the midpoint of the decision boundary
# midpoint = [(mean_class3[0] + mean_class4[0]) / 2, (mean_class3[1] + mean_class4[1]) / 2]
...
# x_vals = np.linspace(np.min(X_pca[:, 0]), np.max(X_pca[:, 0]), 100)
# y_vals = slope * (x_vals - midpoint[0]) + midpoint[1]
# plt.plot(x_vals, y_vals, label='MED Decision Boundary', linestyle='--', color='black')

# plt.title('Decision Boundary for MED Classifier - Training Set')
